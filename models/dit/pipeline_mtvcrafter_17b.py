...
class MTVCrafterPipelineOutput(BaseOutput):
    r"""
    Output class for the MTVCrafter pipeline.
...
class MTVCrafterPipeline17B(DiffusionPipeline):
    r"""
    Pipeline for MTVCrafter-17B.
    ...
    def __call__(
        ...
        motion_tokens: Union[torch.FloatTensor] = None,
        motion_rotary_emb: Union[torch.FloatTensor] = None,
        max_sequence_length: int = 512,
        ...
        if video is not None:
            init_video = rearrange(video, "b f c h w -> b c f h w")
        else:
            init_video = None
        ...
        if use_2d and control_video is not None:
            ...
            #     ).to(device, weight_dtype)
            #     y = torch.cat([control_latents, ref_image_latentes_conv_in], dim = 1)
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype):
                    if lora_path == "" and do_classifier_free_guidance:
                        ...
                        noise_pred = self.transformer(
                            x=latent_model_input[1:2],
                            context=prompt_embeds[1:2],
                            t=timestep[1:2],
                            seq_len=seq_len,
                            y=y[1:2],
